...
export default function CongregationMembersPage() {
  ...
  const congregation = useMemo(
    () => members.filter((m) => m.congregationMember && m.memberStatus !== "제적"),
    [members],
  );

  const filtered = useMemo(() => {
    const result = query ? searchMembers(congregation, query) : congregation;
    return query ? result : sortMembers(result, sortKey);
  }, [congregation, query, sortKey]);
      ...
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur no-print">
        <div className="mx-auto flex h-14 max-w-5xl items-center gap-2 px-4">
          ...
          <Button
            ...
            className="ml-auto h-9 px-3 shrink-0"
          >
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-bold">공동의회회원 명부</h1>
            <Badge variant="secondary">{filtered.length}명</Badge>
            {filtered.length !== congregation.length && (
              <span className="text-xs text-muted-foreground">
                (전체 {congregation.length}명 중)
              </span>
            )}
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="font-medium">
              {congregation.length === 0 ? "등록된 공동의회회원이 없습니다" : "검색 결과가 없습니다"}
            </p>
            {congregation.length === 0 && (
              <p className="mt-1 text-sm text-muted-foreground">
                교인 상세 정보 수정에서 공동의회회원 여부를 &quot;예&quot;로 설정하면 이 명부에 표시됩니다
              </p>
            )}
          </div>
        ) : (
